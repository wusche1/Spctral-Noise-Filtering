...
class Data:
    def __init__(
        ...
        x,
        name=None,
        typical_peak_number=0.408,
        max_peak_percentage=0.95,
        prior_maxwell=1 / 3,
        prior_kelvin_voigt=1 / 3,
        prior_fractional_kelvin_voigt=1 / 3,
        log_weighted=False,
        unweighted_postfit=False,
        typical_peak_length=1,
    ):
        self.t = t
        self.x = x
        self.name = name
        ...
        self.typical_peak_length = typical_peak_length
        self.max_peak_percentage = max_peak_percentage
        self.H_l = None
        self.prior = None
        assert not (
            (not log_weighted) and unweighted_postfit
        ), "Unweighted postfit only makes sence, if you previously weighted the data."
    ...
    def check_typical_peak_length(self):
        assert self.suprise is not None

        prior_length = np.log(2) / self.typical_peak_length
        prior_max_peak = sorted(self.suprise)[
            int(len(self.suprise) * self.max_peak_percentage)
        ]

        self.prior = max([prior_max_peak, prior_length])

        if prior_max_peak > prior_length:
            new_length = np.log(2) / prior_max_peak
            print(
                f"Typical length is too short for dataset for a given maximal peak procentage of {self.max_peak_percentage}. Resetting the typical length to {new_length}"
            )
            self.typical_peak_length = new_length

        self.H_l = calculate_p_l_entropy(
            self.typical_peak_length
        ) - calculate_p_l_entropy_prefactor(self.typical_peak_length)
    ...
    def create_fit_maxwell(self):
        ...
        self.fit_maxwell, self.param_sigma_maxwell = fit_maxwell(
            self.frequencies,
            self.PSD,
            self.initial_guess_maxwell,
            log_weighted=self.log_weighted,
        )

        if self.unweighted_postfit:
            self.fit_maxwell, self.param_sigma_maxwell = fit_maxwell(
                self.frequencies, self.PSD, self.fit_maxwell, log_weighted=False
            )
    ...
    def create_initial_guess_kelvin_voigt(self):
        ...
        self.initial_guess_kelvin_voigt = initial_guess_kelvin_voigt(
            self.frequencies, self.PSD, log_weighted=self.log_weighted
        )
    ...
    def create_fit_kelvin_voigt(self):
        ...
        self.fit_kelvin_voigt, self.param_sigma_kelvin_voigt = fit_kelvin_voigt(
            self.frequencies,
            self.PSD,
            self.initial_guess_kelvin_voigt,
            log_weighted=self.log_weighted,
        )

        if self.unweighted_postfit:
            self.fit_kelvin_voigt, self.param_sigma_kelvin_voigt = fit_kelvin_voigt(
                self.frequencies, self.PSD, self.fit_kelvin_voigt, log_weighted=False
            )
    ...
    def create_NLL_kelvin_voigt(self, log_weighted=False):
        ...
        self.NLL_kelvin_voigt = Laplace_NLL(
    ...
    def create_initial_guess_fractional_kelvin_voigt(self):
        ...
        self.initial_guess_fractional_kelvin_voigt = (
            initial_guess_fractional_kelvin_voigt(self.frequencies, self.PSD)
        )
    ...
    def create_fit_fractional_kelvin_voigt(self):
        ...
        self.fit_fractional_kelvin_voigt, self.param_sigma_fractional_kelvin_voigt = (
            fit_fractional_kelvin_voigt(
                self.frequencies,
                self.PSD,
                self.initial_guess_fractional_kelvin_voigt,
                log_weighted=self.log_weighted,
        ...
        if self.unweighted_postfit:
            (
                ...
                self.param_sigma_fractional_kelvin_voigt,
            ) = fit_fractional_kelvin_voigt(
                self.frequencies,
                self.PSD,
                self.fit_fractional_kelvin_voigt,
    ...
    def create_NLL_fractional_kelvin_voigt(self, log_weighted=False):
        ...
        self.NLL_fractional_kelvin_voigt = Laplace_NLL(
            self.fit_fractional_kelvin_voigt,
            self.frequencies,
            self.PSD,
            target_function,
    ...
    def bayesian_update(self):
        ...
        n_eff = (
            len(self.PSD)
            if not logweight_update
            else np.sum(1 / np.arange(1, len(self.PSD) + 1))
        )
        ...
        BIC_fractional_kelvin_voigt = (
            5 * np.log(n_eff) + 2 * self.NLL_fractional_kelvin_voigt
        )
        ...
        posterior_fractional_kelvin_voigt = (
            np.exp(-BIC_fractional_kelvin_voigt / 2)
            * self.prior_fractional_kelvin_voigt
        )

        # Normalize the posterior probability
        self.posterior_maxwell = posterior_maxwell / (
            posterior_maxwell
            + posterior_kelvin_voigt
            + posterior_fractional_kelvin_voigt
        )
        self.posterior_kelvin_voigt = posterior_kelvin_voigt / (
            posterior_maxwell
            + posterior_kelvin_voigt
            + posterior_fractional_kelvin_voigt
        )
        self.posterior_fractional_kelvin_voigt = posterior_fractional_kelvin_voigt / (
            posterior_maxwell
            + posterior_kelvin_voigt
            + posterior_fractional_kelvin_voigt
        )
        ...
        if (
            self.posterior_maxwell > self.posterior_kelvin_voigt
            and self.posterior_maxwell > self.posterior_fractional_kelvin_voigt
        ):
            ...
            self.posterior_kelvin_voigt > self.posterior_maxwell
            and self.posterior_kelvin_voigt > self.posterior_fractional_kelvin_voigt
        ):
            ...
            self.fit_function = lambda x, params: PSD(
                x, G_fractional_Kelvin_Voigt, params
            )
            self.fit_params = self.fit_fractional_kelvin_voigt
            self.param_sigma = self.param_sigma_fractional_kelvin_voigt

        self.suprise = get_surprise(
            self.frequencies, self.PSD, self.fit_function, self.fit_params
        )
        self.check_typical_peak_length()
    ...
    def refit(self):
        ...
        if (
            self.posterior_fractional_kelvin_voigt > self.posterior_maxwell
            and self.posterior_fractional_kelvin_voigt > self.posterior_kelvin_voigt
        ):
            self.fit_params, self.param_sigma = fit_fractional_kelvin_voigt(
                filtered_freq,
                filtered_PSD,
                self.fit_params,
                log_weighted=self.log_weighted,
            ...
            self.posterior_maxwell > self.posterior_fractional_kelvin_voigt
            and self.posterior_maxwell > self.posterior_kelvin_voigt
        ):
            self.fit_params, self.param_sigma = fit_maxwell(
                filtered_freq,
                filtered_PSD,
                self.fit_params,
                log_weighted=self.log_weighted,
            ...
            self.fit_params, self.param_sigma = fit_kelvin_voigt(
                filtered_freq,
                filtered_PSD,
                self.fit_params,
                log_weighted=self.log_weighted,
    ...
    def find_peaks(self, max_iter=10, report=False):
        self.peaks = find_all_peaks(
            self.suprise,
            self.max_peak_percentage,
            self.typical_peak_number,
            self.typical_peak_length,
            self.prior,
            self.H_l,
        )
    ...
    def create_reconstructed_data(self):
        ...
        peak_PSD_fit = self.fit_function(self.frequencies, self.fit_params)[
            peak_indeces
        ]
        ...
        assert np.allclose(
            self.PSD[peak_indeces], fourier_abs_sq[peak_forward], rtol=0.1
        ) and np.allclose(
            self.PSD[peak_indeces], fourier_abs_sq[peak_backward], rtol=0.1
        ...
        assert (
            np.max(np.imag(reconstructed_trajectory)) < 1e-15
        ), "Imaginary part of reconstructed trajectory is not neglegible"

        self.reconstructed_x = np.real(reconstructed_trajectory) / np.sqrt(
            dt / len(self.x)
        )
    ...
    def create_activity_filtered_PSD(self, E_0):
        ...
        self.activity_filtered_PSD = self.reconstructed_PSD * (
            1 / (1 + active_Energy_in_kb_T)
        )
        ...
        if (
            ...
            self.activity_filtered_fit_params, self.activity_filtered_param_sigma = (
                fit_fractional_kelvin_voigt(
                    self.frequencies,
                    self.activity_filtered_PSD,
                    self.fit_params,
                    log_weighted=self.log_weighted,
                )
            )
    ...
    def plot_psd(self, ax=None):
        ...
        if self.initial_guess_maxwell is not None and self.fit_maxwell is not None:
            psd_initial_guess_maxwell = PSD(
                log_spaced_freqs, G_Maxwell, self.initial_guess_maxwell
            )
            ax.plot(
                log_spaced_freqs,
                psd_initial_guess_maxwell,
                label="Initial Guess (Maxwell)",
                linestyle="--",
            )
        ...
        if (
            self.initial_guess_kelvin_voigt is not None
            and self.fit_kelvin_voigt is not None
        ):
            psd_initial_guess_kelvin_voigt = PSD(
                log_spaced_freqs, G_Kelvin_Voigt, self.initial_guess_kelvin_voigt
            )
            ax.plot(
                log_spaced_freqs,
                psd_initial_guess_kelvin_voigt,
                label="Initial Guess (Kelvin-Voigt)",
                linestyle="--",
            )
            psd_fit_kelvin_voigt = PSD(
                log_spaced_freqs, G_Kelvin_Voigt, self.fit_kelvin_voigt
            )
        ...
        if (
            self.initial_guess_fractional_kelvin_voigt is not None
            and self.fit_fractional_kelvin_voigt is not None
        ):
            psd_initial_guess_fractional_kelvin_voigt = PSD(
                log_spaced_freqs,
                G_fractional_Kelvin_Voigt,
                self.initial_guess_fractional_kelvin_voigt,
            )
            ax.plot(
                log_spaced_freqs,
                psd_initial_guess_fractional_kelvin_voigt,
                label="Initial Guess (Fractional Kelvin-Voigt)",
                linestyle="--",
            )
            psd_fit_fractional_kelvin_voigt = PSD(
                log_spaced_freqs,
                G_fractional_Kelvin_Voigt,
                self.fit_fractional_kelvin_voigt,
            )
            ax.plot(
                log_spaced_freqs,
                psd_fit_fractional_kelvin_voigt,
                label="Fit (Fractional Kelvin-Voigt)",
            )
